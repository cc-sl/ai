    ...
    for phase, txt_file in dirs.items():
        ...
        for img_id in image_ids:
            ...
            for obj in root.iter('object'):
                ...
                if cls_name not in VOC_CLASSES: continue
                cls_id = VOC_CLASSES.index(cls_name)
                
                xmlbox = obj.find('bndbox')
                # 获取边界框坐标并转换为YOLO格式
                x_min, y_min, x_max, y_max = float(xmlbox.find('xmin').text), float(xmlbox.find('ymin').text), float(xmlbox.find('xmax').text), float(xmlbox.find('ymax').text)
...
def create_few_shot_dataset(yolo_root, few_shot_root, num_samples=400):
    ...
    for img_name in all_imgs:
        ...
        if os.path.exists(lbl_path):
            with open(lbl_path, 'r') as f:
                classes_in_img = set(line.split()[0] for line in f.readlines())
    ...
    for img_name in selected_imgs:
        ...
        shutil.copy(os.path.join(src_lbl_dir, lbl_name), os.path.join(dst_lbl_dir, lbl_name))
        
    # 复制测试集(测试集大小样本实验共用)
    if not os.path.exists(os.path.join(few_shot_root, 'images', 'test')):
        shutil.copytree(os.path.join(yolo_root, 'images', 'test'), os.path.join(few_shot_root, 'images', 'test'))
        shutil.copytree(os.path.join(yolo_root, 'labels', 'test'), os.path.join(few_shot_root, 'labels', 'test'))
...
def evaluate_iou_thresholds(model_path, yaml_path, exp_name):
    ...
    for iou, res in eval_results.items():
        print(f"{iou:<10} {res['Precision']:<12.4f} {res['Recall']:<12.4f} {res['mAP@0.5']:<12.4f} {res['mAP@0.5:0.95']:<15.4f}")
...
def visualize_predictions(model_path, yaml_path, exp_name, num_images=5):
    ...
    # 随机选取测试集图片
    test_img_dir = os.path.join(os.path.dirname(yaml_path), 'images', 'test') if os.path.exists(os.path.join(os.path.dirname(yaml_path), 'images', 'test')) else os.path.join(VOC_ROOT, 'JPEGImages')
    test_lbl_dir = os.path.join(os.path.dirname(yaml_path), 'labels', 'test') if os.path.exists(os.path.join(os.path.dirname(yaml_path), 'labels', 'test')) else os.path.join(YOLO_DATASET_ROOT, 'labels', 'test')
    ...
    for img_name in samples:
        ...
        if os.path.exists(lbl_path):
            ...
            for line in lines:
                ...
                cls_id, x_c, y_c, bw, bh = int(parts[0]), float(parts[1]), float(parts[2]), float(parts[3]), float(parts[4])
                # 还原坐标
                ...
                cv2.putText(img, f"GT:{VOC_CLASSES[cls_id]}", (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        ...
        if res.boxes is not None:
            for box in res.boxes:
                ...
                cv2.putText(img, f"{VOC_CLASSES[cls_id]}:{conf:.2f}", (x1, y2+15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
...
if __name__ == "__main__":
    ...
    # 评估IoU阈值影响 
    evaluate_iou_thresholds(few_model_path, full_yaml, "FewShot_yolo11n")
    time5 = time.time()
